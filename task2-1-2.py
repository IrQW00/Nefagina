...
class DataSet:
    ...
    def csv_reader(file_name: str):
        """
                Считывает CSV файл.
                Выводит строку "Пустой файл" и прерывает работу программы, иначе - возвращает все строки из файла
                Args:
...
class Salary:
    """
        ...
        Attributes:
            salary_from (int): Зарплато от
            salary_to (int): Зарплата до
            salary_currency (str): Название валюты
            currency_to_rub (dict): Курс обмена валют
        """
    salary_from: str
    salary_to: str
    salary_currency: str
    ...
    def set_medianSalary(self):
        """
                Считает среднюю зарплату в рублях по формуле (salary_from + salary_to) / 2
        """
...
class DataDictionary:
    """
           Запрашивает у пользователя все необходимые данные и выводит данные статистики вакансий
    """
    def __init__(self):
        """
               Инициализирует объект DataDictionary
        """
        self.salary_years = {}
